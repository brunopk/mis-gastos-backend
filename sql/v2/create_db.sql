...
CREATE TABLE `subcategory_account` (
	...
	CONSTRAINT `fk_subcategory_account_account`
        FOREIGN KEY `subcategory_account`(`account_id`)
        REFERENCES `account`(`id`)
);
...
CREATE TABLE `task_config` (
    ...
    `google_task_description_template` VARCHAR(128),
    `send_mail` TINYINT(1) NOT NULL,
    `mail_subject` TEXT,
    `mail_body_template` VARCHAR(128),
    `category_id` INT UNSIGNED NOT NULL,
    ...
    CONSTRAINT `fk_task_config_category`
        FOREIGN KEY `task_config`(`category_id`)
        REFERENCES `category`(`id`),
    CONSTRAINT `fk_task_config_subcategory`
        FOREIGN KEY `task_config`(`subcategory_id`)
        REFERENCES `subcategory`(`id`),
    CONSTRAINT `fk_task_config_group`
        FOREIGN KEY `task_config`(`group_id`)
        REFERENCES `group`(`id`),
...
CREATE TABLE `spend` (
    ...
    CONSTRAINT `fk_spend_account` 
        ...
        REFERENCES `account`(`id`),
    CONSTRAINT `fk_spend_category`
        FOREIGN KEY `spend`(`category_id`)
        REFERENCES `category`(`id`),
    CONSTRAINT `fk_spend_subcategory`
        FOREIGN KEY `spend`(`subcategory_id`)
        REFERENCES `subcategory`(`id`),
    CONSTRAINT `fk_spend_group` 
    	FOREIGN KEY `spend`(`group_id`)
    	REFERENCES `group`(`id`),
